...
export default function Career() {
    return (
        <section className="career-section w-full py-20 md:py-32 px-4 md:px-8 lg:px-16">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="career-timeline relative">
                    {/* Vertical Line */}
                    <div className="hidden md:block absolute left-8 lg:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-white via-white/50 to-transparent"></div>

                    {/* Jobs */}
                    <div className="space-y-8 md:space-y-12">
                        {jobs.map((job, index) => (
                            <div key={job.id} className={`job-item relative`}>
                                {/* Timeline Dot */}
                                <div className="hidden md:block absolute left-0 lg:left-1/2 top-8 -translate-x-1/2 w-4 h-4 bg-white rounded-full border-4 border-black"></div>

                                {/* Card */}
                                <div className={`${index % 2 === 0 ? 'md:mr-auto md:w-1/2 md:pr-12' : 'md:ml-auto md:w-1/2 md:pl-12'}`}>
                                    <div className="p-6 md:p-8 border border-white/30 bg-black/60 hover:bg-black/80 transition-all duration-300 backdrop-filter backdrop-blur-10px relative overflow-hidden group">
                                        ...
                                        {/* Current Badge */}
                                        {job.current && (
                                            <div className="inline-block mb-4 px-3 py-1 bg-white/20 border border-white rounded text-white font-mono text-xs">
                                                $ active_job
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-start gap-3 mb-4">
                                            <Briefcase className="text-white flex-shrink-0 mt-1" size={20} />
                                            <div className="flex-1">
                                                <h3 className="text-lg md:text-xl font-bold text-white font-mono mb-1">
                                                    {job.title}
                                                </h3>
                                        ...
                                        <div className="border-t border-white/20 pt-4">
                                            <p className="text-gray-400 text-xs font-mono mb-2">
                                                <span className="text-white">&gt;</span> stack_used:
                                            </p>
                                            <div className="flex flex-wrap gap-2">
                                                {job.technologies.map((tech) => (
                                                    <span
                                                        key={tech}
                                                        className="px-3 py-1 text-xs font-mono bg-white/10 border border-white/40 text-white rounded hover:border-white transition-colors"
                                                    >
                                                        {tech}
                                                    </span>
                                                ))}
                ...
                <div className="mt-12 md:mt-16 text-center">
                    <p className="text-gray-400 font-mono text-sm mb-4">
                        <span className="text-white">&gt;</span> Quer saber mais sobre minha jornada?
                    </p>
